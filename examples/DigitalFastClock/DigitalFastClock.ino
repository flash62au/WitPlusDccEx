...
void wifiOnConnect() {
  ...
  if (!client.connect(host.c_str(), port)) {
    ...
    WitPlusDccEx.setDeviceName("mylittlethrottle");
  }
}

void wifiOnDisconnect() {
  WitPlusDccEx.disconnect();
...
void updateFastTimeDisplay()
{
  ...
  int hour = WitPlusDccEx.fastTimeHours();
  int minutes = WitPlusDccEx.fastTimeMinutes();
...
void loop()
{
  ...
  // processing needed in the WitPlusDccEx class (mostly reading data from
  ...
  if (WitPlusDccEx.check()) {
    if (WitPlusDccEx.clockChanged) { updateFastTimeDisplay(); }

    if (WitPlusDccEx.protocolVersionChanged) {
        Serial.print("PROTOCOL VERSION "); Serial.println(WitPlusDccEx.protocolVersion);
    }
  }
}
